...
def nova_empresa(request):
    if request.method == "GET":
        tecnos = Tecnologias.objects.all()
        return render(request, 'nova_empresa.html', {'tecnos': tecnos})
...
def empresas(request):
    tecnolologia_filtrar = request.GET.get('tecnologias')
    nome_filtrar = request.GET.get('nome')
    empresas = Empresa.objects.all()

    if tecnolologia_filtrar:
        empresas = empresas.filter(tecnologias = tecnolologia_filtrar)
...
def empresa(request,id):
    ...
    return render(request, 'empresa.html', {'empresa': empresa, 'tecnologias':tecnologias, 'empresas':empresas, 'vagas':vagas})